...
| CF-C: 12 missing `docs/src/reference/*.md`; `validate-feature-status` CI bypassed | v0.74.0 EVIDENCE-01, GATE-05 |
| CF-D: Datalog/R2RML/CDC write paths bypass mutation journal entirely | v0.74.0 JOURNAL-DATALOG-01 |
| HF-A: SBOM contains stale `pg_ripple@0.51.0`; top-level one release behind | v0.74.0 SBOM-03 |
| HF-B: `pg_ripple_http` version frozen at 0.16.0 | v0.74.0 HTTP-VERSION-01 |
| HF-C: Datalog/R2RML/CDC bypass all mutation-journal-driven side effects | v0.74.0 JOURNAL-DATALOG-01 |
| HF-D: `validate-feature-status` runs on fresh DB, not populated | v0.74.0 GATE-06 |
| HF-E: Mutation journal doc-comment misleads about caller contract | v0.74.0 DOC-JOURNAL-01 |
| HF-F: Plan cache key omits current_role / GUCs (A10 HF-6 unchanged) | v0.72.0 CACHE-01 |
| HF-G: `recover_interrupted_promotions()` not auto-invoked at startup | v0.74.0 PROMO-RECOVER-01 |
| MF-A: Plan cache invalidation on VP promotion not implemented | v0.74.0 CACHE-INVALIDATE-01 |
| MF-B: `pg_ripple_http` COMPATIBLE_EXTENSION_MIN stale | v0.74.0 HTTP-VERSION-01 |
| MF-C: `merge_throughput_history.csv` still one row (A10 MF-5) | v0.72.0 BENCH-03 |
| MF-D: `v070_features.sql` regression test missing | v0.74.0 TEST-04 |
| MF-E: `src/lib.rs` and `src/storage/mod.rs` continued to grow (A10 MF-10) | v0.72.0 MOD-01 |
| MF-F: Per-API flush in `dict_api.rs` quadratic on rule count | v0.74.0 FLUSH-DEFER-01 |
| MF-G: 12 missing docs break operator UX for `feature_status()` | v0.74.0 EVIDENCE-01 |
| MF-H: `unwrap()` outside `#[cfg(test)]` in dictionary/inline.rs, flight.rs, etc. | v0.75.0 UNWRAP-AUDIT-01 |
| MF-I: Citus integration test not in any CI workflow | v0.75.0 CI-INTEGRATION-01 |
| MF-J: Arrow export integration test not in CI | v0.75.0 CI-INTEGRATION-02 |
| MF-K: Roadmap status not validated post-release | v0.75.0 ROADMAP-VALIDATE-01 |
| MF-L: `mutation_journal::flush()` doc-comment contract inaccurate | v0.74.0 DOC-JOURNAL-01 |
| MF-M: `apply_rls_to_vp_table` swallows errors via `let _ =` | v0.75.0 RLS-ERROR-01 |
| MF-N: `is_safe_role_name` rejects non-ASCII identifiers | v0.75.0 ROLE-DOC-01 |
| MF-O: Verify role quoting in `apply_rls_policy_to_all_dedicated_tables` | v0.75.0 RLS-AUDIT-01 |
| MF-P: Property path inside OPTIONAL / GRAPH with vp_rare untested | v0.75.0 PROPPATH-TEST-01 |
| MF-Q: No fuzz target for URL host parser | v0.75.0 FUZZ-URL-01 |
| MF-R: HTTP companion skip-compat env var not documented for production | v0.75.0 COMPAT-DOC-01 |
| MF-S: 60s/run fuzz duration inadequate | v0.75.0 FUZZ-DURATION-01 |
| MF-T: `feature_status.rs` lacks mutation_journal entry | v0.75.0 FEATURE-STATUS-JOURNAL-01 |
| Low: `rust-toolchain.toml` pins "stable" not specific version | v0.76.0 TOOLCHAIN-PIN-01 |
| Low: Policy name 64-bit hash collision risk | v0.76.0 RLS-HASH-01 |
| Low: `arrow` dep not pinned to minor | v0.76.0 ARROW-PIN-01 |
| Low: `merge_throughput_baselines.json` baseline v0.53.0 | v0.76.0 BENCH-REFRESH-01 |
| Low: Test count ~190 vs 300+ target | v0.76.0 TEST-GROWTH-01 |
| Low: `/metrics` auth model undocumented | v0.76.0 METRICS-AUTH-DOC-01 |
| Low: xact callback PRE_COMMIT SPI safety claim lacks citation | v0.76.0 XACT-SPI-DOC-01 |
| Low: No `RegisterEmitLogHook` for secrets suppression | v0.76.0 LOG-HOOK-01 |
| Low: `clippy --deny warnings` gate not re-verified | v0.76.0 CLIPPY-VERIFY-01 |
| Low: `src/llm/` and `src/kge.rs` not in feature_status | v0.73.0 FEATURE-STATUS-02 (verify in v0.76.0) |
| Low: Integration scripts not in CI workflows | v0.75.0 CI-INTEGRATION-01, CI-INTEGRATION-02 |
...
| A10 MF-18: URL-host parser untested | v0.72.0 CITUS-URL-01, v0.75.0 FUZZ-URL-01 |
...
| Recommended action 3: Datalog/R2RML/CDC mutation journal wiring | v0.74.0 JOURNAL-DATALOG-01 |
| Recommended action 4: 12 missing docs or strip citations | v0.74.0 EVIDENCE-01 |
| Recommended action 5: SBOM regeneration + deduplication | v0.74.0 SBOM-03 |
| Recommended action 6: HTTP companion version alignment | v0.74.0 HTTP-VERSION-01 |
| Recommended action 7: v070_features.sql | v0.74.0 TEST-04 |
| Recommended action 8: Per-statement flush deferral | v0.74.0 FLUSH-DEFER-01 |
| Recommended action 9: Integration tests in CI | v0.75.0 CI-INTEGRATION-01, CI-INTEGRATION-02 |
| Recommended action 10: Plan cache reset on promotion | v0.74.0 CACHE-INVALIDATE-01 |
| Maturity area: Correctness (C rating → target B) | v0.74.0 CF-A, v0.74.0 CF-C/CF-D |
| Maturity area: Code Quality (C+ → target B) | v0.72.0 MOD-01, v0.75.0 UNWRAP-AUDIT-01 |
| Maturity area: Documentation (C+ → target B) | v0.74.0 EVIDENCE-01, v0.76.0 METRICS-AUTH-DOC-01 |
| Maturity area: Performance (C+ → target B) | v0.74.0 FLUSH-DEFER-01, CACHE-INVALIDATE-01 |
