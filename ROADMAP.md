...
### Assessment 13 Remediation (v0.83.1 – v0.83.3)

| Version | Theme | Status | Scope | Full details |
|---------|-------|--------|-------|--------------|
| [v0.83.1](roadmap/v0.83.1.md) | Assessment 13 critical/high & operational remediation: HTTP companion version sync — bump pg_ripple_http to 0.83.0 and raise `COMPATIBLE_EXTENSION_MIN` to `"0.83.0"` (HTTP-01/MF-B); docker-compose image tag updated to 0.83.0 with CI version-sync lint (BUILD-01); SECURITY DEFINER inline justification comments for all three occurrences + update check_no_security_definer.sh allow-list (S13-01); wire `check_no_string_format_in_sql.sh` as required CI step (S13-02); migration-chain test extended with v0.80.0–v0.83.0 checkpoints (T13-01); nested OPTIONAL + EXISTS filter-pushdown regression test and fix (C13-01); split `src/gucs/registration.rs` (2 032 lines) into per-domain submodules: `registration/{sparql,storage,federation,datalog,security,observability}.rs` (Q13-01); `/health/ready` endpoint extended with real DB reachability probe distinct from liveness (O13-01); `cache_key()` avoids double-parse — pre-parsed `&spargebra::Query` threaded through call sites (P13-01); `audit.toml` expiry dates added and `serde_cbor` advisory resolution documented (S13-04); `PG_RIPPLE_HTTP_STRICT_COMPAT` env-var for fail-closed compat check in CI (S13-05); `justfile` gains `bump-version`, `regen-sbom`, `regen-openapi` recipes (BUILD-02); migration-chain smoke test wired as CI workflow step (BUILD-03) | Planned | Large | [Full details](roadmap/v0.83.1-full.md) |
| [v0.83.2](roadmap/v0.83.2.md) | Assessment 13 correctness, performance & code quality: `batch_decode` respects `pg_ripple.strict_dictionary` GUC — errors on missing IDs rather than silently emitting empty strings (C13-02); `execute_drop` / `execute_clear` add pre-condition assertion that mutation journal is open before modifying storage (C13-04); plan cache key normalises `INFERENCE_MODE` via trim+lowercase (C13-05); `batch_decode` warning guard tightened to `id == 0` rather than `id <= 0` (C13-07); `encode_token` in `magic.rs` handles typed literals with proper language/datatype encoding (C13-08); `parse_nt_triple` adds IRI length and termination bounds checks (C13-09); `xsd:dateTime` sub-millisecond precision documented in reference docs (C13-10); `describe_cbd` recursion depth capped via new `pg_ripple.describe_depth_limit` GUC (C13-11); RDF-star blank-node-in-quoted-triple canonical form documented and round-trip tested (C13-03); `GRAPH ?g` default-graph exclusion documented and regression tested (C13-06); encode batch API reduces per-triple SPI overhead in hot paths (P13-02); merge-worker heartbeat log throttled to avoid log spam (P13-03); `SET LOCAL` calls batched into single SPI round-trip (P13-04); intra-stratum cycle pre-check guards SCC evaluator (P13-06); HOT-path metrics registered in Prometheus for plan-cache and encode paths (P13-08); `src/schema.rs` (1 939 lines) split into `schema/{tables,indices,functions,triggers}.rs` (Q13-02); `src/sparql/federation.rs` (1 693 lines) split into `federation/{planner,executor,cache,cost}.rs` (Q13-03); CI lint gate rejects any new file exceeding 1 800 lines (Q13-04); `#[allow(dead_code)]` annotations audited — retained only where upstream crates require it (Q13-05); VP-promotion crash-recovery regression test verifying catalogue consistency after simulated crash (CC13-01); merge fence lock scoped to per-predicate rather than global (CC13-02) | Planned | Large | [Full details](roadmap/v0.83.2-full.md) |
| [v0.83.3](roadmap/v0.83.3.md) | Assessment 13 tests, API polish, observability, supply chain & standards: `sparql_roundtrip` proptest suite compares pg_ripple results against the `spargebra` reference evaluator (T13-02); CONSTRUCT + SHACL-SPARQL fuzz targets added to scheduled fuzz CI (T13-03); W3C / Jena / WatDiv / LUBM conformance trends committed as CSV artifact on each CI run (T13-04); `pg_extern` coverage gap audit — any untested public entry points gain regression tests (T13-05); benchmark regression gate added to CI — fails if throughput drops > 10 % vs baseline (T13-06); CDC slot cleanup crash-recovery test verifies slot state after simulated worker crash (T13-07); `load_jsonld` removal scheduled in CHANGELOG with a concrete target version (A13-01); OpenAPI YAML committed to `docs/openapi/` and validated in CI (A13-02); `docs/src/reference/error-codes.md` PT-code registry added — all `PT-xxx` error strings cross-referenced (A13-03); deprecated GUCs documented in reference guide with replacement GUC names (A13-04); SPARQL error response codes standardised across HTTP companion and extension (A13-06); compatibility matrix updated to cover v0.80–v0.83 HTTP companion versions (D13-01); feature reference docs updated for all v0.80–v0.83 additions (D13-02); SPARQL 1.2 tracking section updated with latest spec changes (D13-03); `parquet` → 60, `ureq` → 3, `arrow` → 56 dependency upgrades and SBOM regenerated (DS13-01); SBOM regeneration runs as required CI step (DS13-03); `rust-toolchain.toml` channel updated to latest stable (DS13-04); `tokio-stream` evaluated — either streaming cursor prototype landed or dependency removed (DS13-05/RR-01); federation and dictionary-cache hit/miss metrics exposed in Prometheus (O13-02); `explain_sparql()` output gains `algebra_after_opt` field showing post-optimiser algebra tree (O13-03); `pg_ripple_http` supports `PG_RIPPLE_LOG_FORMAT=json` for structured log output (O13-04); axum server registers graceful shutdown on `SIGTERM` (O13-05); `sparql_strict` GUC behaviour verified with regression test for each covered filter case (SC13-01); `SERVICE SILENT` error-swallowing extended to TLS failures, oversized responses, and redirect loops (SC13-02); GeoSPARQL function inventory documented (SC13-03); `DESCRIBE` algorithm documented and `pg_ripple.describe_form` GUC added (SC13-04); CORS policy violations emit a Prometheus counter (S13-03); bulk-loader file paths canonicalised before `open()` (S13-06); docker-compose.yml uses random generated passwords (S13-07); Arrow Flight bulk-export endpoint returns HTTP 413 before materialising when estimated row count exceeds limit (S13-08); metrics endpoint documentation clarifies authentication requirement (S13-09); auth scheme documentation added to HTTP companion README (S13-10); Datalog rule streaming in batches reduces peak memory for large rule sets (P13-05); `PathCtx` visibility tightened to module boundary (P13-07); CDC LSN watermark updates batched to reduce write amplification (CC13-03); blog post index added to `blog/README.md` (D13-04); SPARQL 1.2 proposals linkage added to ROADMAP narrative (D13-03 ext) | Planned | Large | [Full details](roadmap/v0.83.3-full.md) |

### Assessment 13 Remediation (v0.84.0 – v0.86.0)

| Version | Theme | Status | Scope | Full details |
|---------|-------|--------|-------|-------------- |
| [v0.84.0](roadmap/v0.84.0.md) | Assessment 13 critical/high & operational remediation: HTTP companion readiness, plan cache double-parse fix, GUC module split, SQL-injection CI lint, migration-chain test extension, justfile recipes, SECURITY DEFINER annotations, audit.toml expiry dates | Planned | Large | [Full details](roadmap/v0.84.0-full.md) |
| [v0.85.0](roadmap/v0.85.0.md) | Assessment 13 correctness, performance & code quality: strict decode, mutation journal assertions, plan cache normalisation, IRI length bounds, encode batch API, merge-worker throttling, cycle pre-check, HOT-path metrics, schema.rs/federation.rs splits, per-file line-count CI gate, per-predicate merge fence | Planned | Large | [Full details](roadmap/v0.85.0.md) |
| [v0.86.0](roadmap/v0.86.0.md) | Assessment 13 tests, API polish, observability & security: sparql_roundtrip proptest, CONSTRUCT/SHACL-SPARQL fuzz, conformance trend artifacts, benchmark regression gate, OpenAPI CI, error-code registry, structured JSON logs, axum graceful shutdown, dependency upgrades, CORS counter, Arrow 413 guard | Planned | Large | [Full details](roadmap/v0.86.0.md) |
...
untested `pg_extern` functions. v0.83.1 through v0.83.3 address all 82 findings
from PLAN_OVERALL_ASSESSMENT_13: v0.83.1 closes the ten "must-fix before v1.0.0"
items — HTTP companion version synchronisation, docker-compose image tag, SECURITY
DEFINER inline justification, CI format-check gate, migration-chain checkpoints for
v0.80–v0.83, nested OPTIONAL+EXISTS fix, `gucs/registration.rs` split, `/health/ready`
deep-check, plan cache double-parse elimination, and `justfile` automation recipes;
v0.83.2 delivers correctness and code-quality improvements — `strict_dictionary` in
...
crash-recovery regression test; v0.83.3 closes the remaining 30+ backlog items across
...
v0.84.0 delivers the uncertain knowledge engine:
