...
**Theme**: GeoSPARQL 1.1, SPARQL Entailment Regime conformance tests, Apache Arrow/Flight bulk export, federation circuit breaker, DDL event trigger, SPARQL audit log, whole-file dead-code audit, and removal of the deprecated `property_path_max_depth` GUC. The dbt adapter is deferred to [v0.59.0](v0.59.0-full.md).

> **In plain language:** Building on the PostGIS bundling in v0.54.0, this release wires the full GeoSPARQL 1.1 geometry function vocabulary so users can ask “find all hospitals within 5 km of this point” directly in SPARQL. A formal conformance test suite for SPARQL Entailment Regimes verifies that the OWL 2 RL + SPARQL combination behaves correctly per the W3C spec. A native Arrow Flight endpoint lets data engineers stream large graph exports at columnar speed. The federation circuit breaker prevents one failing remote endpoint from degrading the entire query. The dbt adapter (`dbt-pg-ripple`) is deferred to [v0.59.0](v0.59.0-full.md).
>
