...
- [x] XACT-01: SAVEPOINT rollback test passes; no phantom derived triples.
- [x] CACHE-01: Plan cache docs created; role-isolation test passes.
- [x] MOD-01: Three target files ≤ 500 lines each; no pg_regress regressions.
- [x] PROPTEST-01: `cargo test --features pg18` includes proptest suite with ≥ 100 cases.
- [x] FUZZ-02: 13th fuzz target present; seeds included.
- [x] CONF-01: W3C syntax/basic/optional/graph subsets and BSBM are blocking CI steps.
- [x] BENCH-03: History CSV has ≥ 4 rows; weekly workflow appends rows.
- [x] FLIGHT-NONCE-01: Replay rejection HTTP test passes.
- [x] CWB-DATALOG-01: Datalog → CWB interaction test passes.
- [x] OBS-02: `/metrics/extension` route returns `streaming_metrics()` JSON.
- [x] GUC-DOC-01: GUC reference doc created.
- [x] CITUS-URL-01: URL parsing edge-case tests pass.
- [x] JSONLD-NODE-01: `export_jsonld_node()` function exists; recursive strip works on nested blank nodes; pg_regress tests pass; `docs/src/operations/pg-trickle-relay.md` Step 5 updated to use the new function.
- [x] BUG-JSONLD-CONTEXT-01: object-form context entries handled for `@container: @list`, `@type`, `@type: @id`, and `@language`; round-trip tests pass for all four.
- [x] RT-FIX-04B: i64 overflow at ingest raises a descriptive error or stores via string preservation path; regression test passes.
- [x] RT-FIX-06: float `5.0` stored as `xsd:decimal`, integer `5` stored as `xsd:integer`; regression test passes.
- [x] RT-FIX-07: invalid IRI components under `@vocab` raise a descriptive error suggesting an explicit context entry; regression test passes.
- [x] RT-TEST-02: empty object round-trip regression test passes.
- [x] All 190+ pg_regress tests pass.
